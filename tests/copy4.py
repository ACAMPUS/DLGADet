...
def load_img_from_train_v2(img_list,k=3):
    """
        从train中随机选择batch张图
        """
    # 被选中的
...
while len(img_list)!=0:
    ...
    for img,labels in zip(five_imgs,five_labels):
        ...
        labels[:, 1:] = bbox
        # todo:修改数据文件夹
        target_path=img_path_list[j].replace('test','test_new').replace('images','labels').replace(suffix,'.txt')
        ...
        cv2.imwrite(img_path_list[j].replace('test','test_new'),img,[int(cv2.IMWRITE_PNG_COMPRESSION),0])

        image_path=img_path_list[j].replace('test','test_new')
        label_path=img_path_list[j].replace('test','test_new').replace('images','labels').replace(suffix,'.txt')
        #
