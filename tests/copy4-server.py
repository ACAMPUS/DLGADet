...
def create_copy_label(h, w, l, labels,s):
    ...
    for epoch in range(30):
        ...
        if xmin < 0 or xmax > w or ymin < 0 or ymax > h:
            continue
        new_l = np.array([l[0], xmin, ymin, xmax, ymax]).astype(np.int).flatten()
        if (new_l[4] - new_l[2])!=l_h or (new_l[3] - new_l[1])!=l_w:
            continue
...
while len(img_list)!=0:
    leave_list,annotations,select_img_list,img_path_list,image_index_dict=load_img_from_train(train_img_path,img_list)
    select_main_labels=transform_annotations(annotations)
    # origin_img_labels=transform_annotations_dict(image_index_dict)
    ...
    for img_path,select_main_label in zip(img_path_list,select_main_labels):
        ...
        #     print()
        labels=np.array(select_main_label).reshape(-1, 5)
        size = img.T.shape[-2:]
        bbox = labels[:, 1:]
    ...
    # 为5张图粘贴five_labels中的图
    j=0
    for img,labels in zip(five_imgs,five_labels):
        h, w = img.shape[0], img.shape[1]
        wh=np.array([w,h])
        # if h==540 and w==810:
        #     print()
        for i in range(len(five_labels)):
            s=random.choice(result)
            # s=1
            l=five_labels[i].flatten()
            l_h, l_w = ((l[4] - l[2])*s).astype(np.int), ((l[3] - l[1])*s).astype(np.int)
            new_label=create_copy_label(h, w, l, labels,s)
            if (new_label[4] - new_label[2]) != l_h or (new_label[3] - new_label[1]) != l_w:
                continue
        ...
        labels[:, 1:] = bbox
        # todo:修改数据文件夹
        target_path=img_path_list[j].replace('train_old','train_select_5').replace('images','labels').replace(suffix,'.txt')
        ...
        cv2.imwrite(img_path_list[j].replace('train_old','train_select_5'),img,[int(cv2.IMWRITE_PNG_COMPRESSION),0])

        image_path=img_path_list[j].replace('train_old','train_select_5')
        label_path=img_path_list[j].replace('train_old','train_select_5').replace('images','labels').replace(suffix,'.txt')
        #
