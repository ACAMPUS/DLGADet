...
def run(data,
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        # Inference
        model.half()
        out, train_out = model(im.half()) if training else model(im.half(), augment=augment, val=True)  # inference, loss outputs
        dt[1] += time_sync() - t2
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / r'D:\dataset\实验记录\38\exp40\weights\best.pt', help='model.pt path(s)')
