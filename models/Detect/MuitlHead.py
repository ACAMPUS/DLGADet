...
class Decoupled_DetectWithAttn(nn.Module):
    stride = None  # strides computed during build
    onnx_dynamic = False  # ONNX export parameter
    export = False  # export mode
...
class ASFF_Detect(nn.Module):   #add ASFFV5 layer and Rfb 
    ...
    def __init__(self, nc=80, anchors=(), ch=(), multiplier=1,rfb=False,inplace=True):  # detection layer
        ...
        self.na = len(anchors[0]) // 2  # number of anchors
        self.grid = [torch.zeros(1)] * self.nl  # init grid
        self.l0_fusion = ASFFV5_4LEVEL(level=0, multiplier=multiplier,rfb=rfb)
        self.l1_fusion = ASFFV5_4LEVEL(level=1, multiplier=multiplier,rfb=rfb)
        self.l2_fusion = ASFFV5_4LEVEL(level=2, multiplier=multiplier,rfb=rfb)
        self.l3_fusion = ASFFV5_4LEVEL(level=3, multiplier=multiplier,rfb=rfb)
