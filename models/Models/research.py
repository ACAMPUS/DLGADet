...
class DecoupledHeadWithAttn(nn.Module):
    def __init__(self, ch=256, nc=80,  anchors=()):
        super().__init__()
        self.nc = nc  # number of classes
        self.nl = len(anchors)  # number of detection layers
...
class ASFFV5(nn.Module):
    ...
    def forward(self, x): #l,m,s
        ...
        levels_weight = F.softmax(levels_weight, dim=1) # （1，3,80,80） 3->α，β，γ
...
class ASFFV5_4LEVEL(nn.Module):
    def __init__(self, level, multiplier=1, rfb=False, vis=False, act_cfg=True):
        ...
        For even smaller, you need change code manually.
        """
        super(ASFFV5_4LEVEL, self).__init__()
        self.level = level
        self.dim = [int(1024 * multiplier), int(512 * multiplier),
    ...
    def forward(self, x):  # l,m,s
        ...
        x_level_1 = x[2]  # m
        x_level_2 = x[1]  # s
        x_level_3 = x[0]  # ss  # 160x160
        # print('x_level_0: ', x_level_0.shape)
        # print('x_level_1: ', x_level_1.shape)
        # print('x_level_2: ', x_level_2.shape)
        if self.level == 0: # 20x20
            level_0_resized = x_level_0
            level_1_resized = self.stride_level_1(x_level_1)
            level_2_downsampled_inter = F.max_pool2d(
                x_level_2, 3, stride=2, padding=1)
            level_2_resized = self.stride_level_2(level_2_downsampled_inter)
            level_3_downsampled_inter1 = F.max_pool2d(
                x_level_3, 3, stride=2, padding=1)
            level_3_downsampled_inter2 = F.max_pool2d(
                level_3_downsampled_inter1, 3, stride=2, padding=1)
            level_3_resized = self.stride_level_3(level_3_downsampled_inter2)
        elif self.level == 1: # 40x40
            level_0_compressed = self.compress_level_0(x_level_0)
            level_0_resized = F.interpolate(
                level_0_compressed, scale_factor=2, mode='nearest')
            level_1_resized = x_level_1
        ...
        level_1_weight_v = self.weight_level_1(level_1_resized)
        level_2_weight_v = self.weight_level_2(level_2_resized)
        level_3_weight_v = self.weight_level_2(level_3_resized)
        # print('level_0_weight_v: ', level_0_weight_v.shape)
        # print('level_1_weight_v: ', level_1_weight_v.shape)
        # print('level_2_weight_v: ', level_2_weight_v.shape)
